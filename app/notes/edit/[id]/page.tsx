...
const Page = async ({ params }: { params: { id: string } }) => {
  ...
  return (
    <section className="w-full flex justify-center items-center flex-col relative">
      <Link
        href={`/notes/${id}`}
        className="my-3 border border-border rounded-md p-2 text-gray absolute right-16 top-0 flex items-center justify-center transition-colors hover:bg-dark-gray-accent hover:text-white"
      >
        <ChevronLeft size={20} />
        Go Back
      </Link>
